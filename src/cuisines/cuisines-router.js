...
cuisinesRouter.route("/").get((req, res, next) => {
...
cuisinesRouter.route("/:cuisine_id").get((req, res, next) => {
  const knexInstance = req.app.get("db");
  cuisinesService
    .getById(knexInstance, req.params.cuisine_id)
    .then((cuisine) => {
      if (!cuisine) {
        return res.status(404).json({
          error: {
            message: `Sorry, we don't offer meals from that region just yet!`,
          },
        });
      }
