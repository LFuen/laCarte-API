...
mealsRouter.route("/:meal_id").get((req, res, next) => {
...
mealsRouter.route("/:meal_id/:origin").get((req, res, next) => {
  const knexInstance = req.app.get("db");
  MealsService.getByOrder(knexInstance, req.params.meal_id, req.params.origin)
    .then((meal) => {
      if (!meal) {
        return res.status(404).json({
          error: {
            message: `Sorry, we don't offer dishes from that region just yet!`,
          },
        });
      }
      res.json(serializeMeal(meal));
